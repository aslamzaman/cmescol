...
export const POST = async (Request) => {
    try {
        ...
        */
        ...
        sheet.cell('A5').value(`Unit: ${data.unitFullName}`).style({ fontSize: 10, horizontalAlignment: 'left', verticalAlignment: 'center' });
        ...
        const bill = data.detail;
        const splitBlill = bill.split("+");
        let rowNo = 0;
        items.forEach((item, i) => {

            const billNoTwoDigitString = "0" + (i + 1);
            const billNoTwoDigit = billNoTwoDigitString.substring(billNoTwoDigitString.length - 2, 2);
            const str = `CMES-${data.unit}-${data.q}-${formatedDate(data.dt)}-Bill-${billNoTwoDigit}`
        ...
        sheet.cell('D7').value(`PDF-Bill-CMES-${data.unit}-${data.q}-${data.dt}.pdf`).style({ fontSize: 10, border: true, wrapText: true, horizontalAlignment: 'center', verticalAlignment: 'center' });
        // Border
